...
def train(opt_path):
    ...
    for e in range(1, 42*opt.steps):
        for i, data in enumerate(train_loader):
            ...
            # T(G(A))
            TGA_f = grid_sample(GA_f, grid_A.detach())
            # G(B)
            ...
            gta_tga_distance_G = opt.coef_mspc*F.l1_loss(GTA_f, TGA_f)
            ...
            TGA_f = grid_sample(GA_f.detach(), grid_A)
            logits_B = netD(B*M_B)
            logits_GA = netD(GA_f.detach())
            logits_TB = netD(TB*M_TB)
            logits_GTA = netD(GTA_f)

            gta_tga_distance_D = opt.coef_mspc*F.l1_loss(GTA_f.detach(), TGA_f)
