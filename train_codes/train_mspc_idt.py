...
def train(opt_path):
    ...
    log_items_train = [
        'total_step','lr', 'loss_G_GA', 'loss_G_GTA', 'loss_G_idt', 'loss_G_idt_purturbation', 'gta_tga_distance_G', 'loss_G',
    ...
    for e in range(1, 42*opt.steps):
        for i, data in enumerate(train_loader):
            ...
            loss_G_idt_purturbation = opt.coef_idt*F.l1_loss(TB.detach(), GTB)
            gta_tga_distance_G = opt.coef_mspc*F.l1_loss(GTA, TGA)
            
            loss_G = loss_G_GA + loss_G_GTA + loss_G_idt + loss_G_idt_purturbation + gta_tga_distance_G
            loss_G.backward()
