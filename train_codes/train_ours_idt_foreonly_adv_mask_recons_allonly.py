...
def train(opt_path):
    ...
    log_items_train = [
        'total_step','lr', 'loss_G_GA', 'loss_G_GTA', 'gta_tga_distance_G', 'gta_tga_distance_G_mask', 'loss_G_TM_GA', 'loss_G_TM_GTA', 'loss_G_idt', 'loss_G_idt_pert', 'loss_G_idt_pert', 'loss_G_idt_mask', 'loss_G_idt_mask_pert', 'loss_G_recons', 'loss_G',
    ...
    for e in range(1, 42*opt.steps):
        for i, data in enumerate(train_loader):
            ...
            # Training G
            set_requires_grad([netD, netP, netD_mask], False)
            netG.zero_grad()
            logits_GA = netD(GA)
            logits_GTA = netD(GTA)
            ...
            loss_G = loss_G_GA + loss_G_GTA + gta_tga_distance_G + gta_tga_distance_G_mask + loss_G_TM_GA + loss_G_TM_GTA + loss_G_idt + loss_G_idt_pert + loss_G_idt_pert + loss_G_idt_mask + loss_G_idt_mask_pert + loss_G_recons
            loss_G.backward()
            ...
            # Training D
            set_requires_grad([netD, netP, netD_mask], True)
            netP.zero_grad()
            netD.zero_grad()
            netD_mask.zero_grad()
            ...
            loss_D = loss_D_B + loss_D_GA + loss_D_TB + loss_D_GTA + gta_tga_distance_D + gta_tga_distance_D_mask + loss_pert_constraint_D + loss_D_M_B + loss_D_TM_GA + loss_D_M_TB + loss_D_TM_GTA
