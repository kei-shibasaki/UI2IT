...
def train(opt_path):
    ...
    log_items_train = [
        'total_step','lr', 'loss_G_GA', 'loss_G_GTA', 'gta_tga_distance_G', 'loss_G_background_recons', 'loss_G',
    ...
    network_module_D_pert = importlib.import_module(opt.network_module_D_pert)
    network_module_T = importlib.import_module(opt.network_module_T)
    ...
    netD_pert = getattr(network_module_D_pert, opt.model_type_D_pert)(opt.netD_pert).to(device)
    netP = PerturbationNetwork(**opt.netP).to(device)
    netT = getattr(network_module_T, opt.model_type_T)(opt.netT).to(device)
    ...
    optimD_pert = torch.optim.Adam(netD_pert.parameters(), lr=opt.learning_rate, betas=opt.betas)
    schedulerD_pert = LinearLRWarmup(optimD_pert, opt.lr_w, opt.lr_max, opt.lr_min, opt.step_w, opt.step_max)

    state_dict = torch.load(opt.netT_path, map_location=device)
    netT.load_state_dict(state_dict['netG_state_dict'], strict=True)
    if opt.pretrained_path:
        ...
        netD_pert.load_state_dict(state_dict['netD_pert_state_dict'], strict=True)
        ...
        optimD_pert.load_state_dict(state_dict['optimD_pert_state_dict'])
        ...
        schedulerD_pert.load_state_dict(state_dict['schedulerD_pert_state_dict'])
    ...
    set_requires_grad([netT], False)
    for e in range(1, 42*opt.steps):
        for i, data in enumerate(train_loader):
            A = data['A'].to(device)
            B = data['B'].to(device)
            M_A = data['mask_A'].to(device)
            TM_GA = netT(torch.cat([M_A, A], dim=1))
            # G(A)
            GA_f, GA_b, GA = netG(A, M_A, TM_GA)
            # T(A)
            grid_A, TA, constraint_A, cordinate_contraint_A = netP(A)
            M_TA = grid_sample(M_A, grid_A.detach())
            TM_GTA = grid_sample(M_A, grid_A.detach())
            # G(T(A))
            GTA_f, GTA_b, GTA = netG(TA.detach(), M_TA.detach(), TM_GTA.detach())
            # T(G(A))
            ...
            # Training G
            set_requires_grad([netD, netP, netD_pert], False)
            netG.zero_grad()
            logits_GA = netD(GA)
            logits_GTA = netD_pert(GTA)
            ...
            mask_for_recons = torch.clip(M_A+TM_GA, 0.0, 1.0)
            loss_G_background_recons = opt.coef_recons*F.l1_loss(GA_b*(1-mask_for_recons), A*(1-mask_for_recons))
            
            loss_G = loss_G_GA + loss_G_GTA + gta_tga_distance_G + loss_G_background_recons
            loss_G.backward()
            ...
            # Training D
            set_requires_grad([netD, netP, netD_pert], True)
            netP.zero_grad()
            netD.zero_grad()
            netD_pert.zero_grad()

            GTA_f, GTA_b, GTA = netG(TA, M_TA, TM_GTA)
            TGA = grid_sample(GA.detach(), grid_A)
            TM_TGA = grid_sample(TM_GA.detach(), grid_A)
            logits_B = netD(B)
            logits_GA = netD(GA.detach())
            logits_TB = netD_pert(TB)
            logits_GTA = netD_pert(GTA)
            ...
            if opt.use_grad_clip: torch.nn.utils.clip_grad_norm_(netD_pert.parameters(), opt.grad_clip_val)
            optimD.step()
            optimP.step()
            optimD_pert.step()
            schedulerD.step()
            schedulerD_pert.step()
            ...
            if total_step%opt.eval_freq==0:
                ...
                for i, data in enumerate(val_loader):
                    ...
                    with torch.no_grad():
                        ...
                        grid_A, TA, constraint_A, cordinate_contraint_A = netP(A)
                        M_TA = grid_sample(M_A, grid_A.detach())
                        TM_GTA = grid_sample(TM_GA, grid_A.detach())
                        GTA_f, GTA_b, GTA = netG(TA, M_TA, TM_GTA)
                ...
                if fid_score <= best_fid:
                    ...
                    torch.save({
                        ...
                        'schedulerD_pert_state_dict': schedulerD_pert.state_dict(),
