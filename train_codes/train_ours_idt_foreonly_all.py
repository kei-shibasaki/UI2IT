...
def train(opt_path):
    ...
    log_items_train = [
        'total_step','lr', 'loss_G_GA', 'loss_G_GTA', 'gta_tga_distance_G', 'loss_G_idt', 'loss_G_idt_pert', 'loss_G',
    ...
    for e in range(1, 42*opt.steps):
        for i, data in enumerate(train_loader):
            ...
            netG.zero_grad()
            logits_GA = netD(torch.cat([GA, TM_GA, GA_f], dim=1))
            logits_GTA = netD(torch.cat([GTA, TM_GTA, GTA_f], dim=1))
            ...
            loss_G = loss_G_GA + loss_G_GTA + gta_tga_distance_G + loss_G_idt + loss_G_idt_pert
            loss_G.backward()
            ...
            logits_B = netD(torch.cat([B, M_B, B*M_B], dim=1))
            logits_GA = netD(torch.cat([GA.detach(), TM_GA.detach(), GA_f.detach()], dim=1))
            logits_TB = netD(torch.cat([TB, M_TB, TB*M_TB], dim=1))
            logits_GTA = netD(torch.cat([GTA, TM_GTA, GTA_f], dim=1))
