...
def train(opt_path):
    ...
    log_items_train = [
        'total_step','lr', 'loss_G_GA', 'loss_G_GTA', 'gta_tga_distance_G', 'gta_tga_distance_G_mask', 'loss_G_TM_GA', 'loss_G_TM_GTA', 
        'loss_G_idt', 'loss_G_idt_pert', 'loss_G_idt_pert', 'loss_G_idt_mask_pert', 'loss_G_recons', 'loss_G',
        'loss_D_B', 'loss_D_GA', 'loss_D_TB', 'loss_D_GTA', 'gta_tga_distance_D', 'gta_tga_distance_D_mask', 'loss_pert_constraint_D', 
        'loss_D_M_B', 'loss_D_TM_GA', 'loss_D_M_TB', 'loss_D_TM_GTA', 'loss_D'
    ]
    log_items_val = [
    ...
    netD_back = getattr(network_module_D, opt.model_type_D)(opt.netD).to(device)
    ...
    optimD_back = torch.optim.Adam(netD_back.parameters(), lr=opt.learning_rate, betas=opt.betas)
    schedulerD_back = LinearLRWarmup(optimD_back, opt.lr_w, opt.lr_max, opt.lr_min, opt.step_w, opt.step_max)
    ...
    if opt.pretrained_path:
        ...
        netD_back.load_state_dict(state_dict['netD_back_state_dict'], strict=True)
        ...
        optimD_back.load_state_dict(state_dict['optimD_back_state_dict'])
        ...
        schedulerD_back.load_state_dict(state_dict['schedulerD_back_state_dict'])
    ...
    for e in range(1, 42*opt.steps):
        for i, data in enumerate(train_loader):
            ...
            # Training G
            set_requires_grad([netD, netP, netD_mask, netD_back], False)
            netG.zero_grad()
            logits_GA = netD(GA_f)
            ...
            gta_tga_distance_G_mask = opt.coef_mspc*F.l1_loss(TM_GTA, TM_TGA)
            loss_G_idt = opt.coef_idt*F.l1_loss(B*M_B, GB_f)
            loss_G_idt_pert = opt.coef_idt*F.l1_loss(TB.detach()*M_TB.detach(), GTB_f)
            loss_G_idt_mask = opt.coef_idt*F.l1_loss(M_B, TM_GB)
            loss_G_idt_mask_pert = opt.coef_idt*F.l1_loss(M_TB.detach(), TM_GTB)
            ...
            logits_GA_back = netD_back(GA_b)
            logits_GTA_back = netD_back(GTA_b)
            loss_G_GA_back = opt.coef_adv*loss_fn(logits_GA_back, target_is_real=True)
            loss_G_GTA_back = opt.coef_adv*loss_fn(logits_GTA_back, target_is_real=True)
            gta_tga_distance_G_back = opt.coef_mspc*F.l1_loss(GTA_b, TGA*(1-TM_TGA.detach()))
            loss_G_idt_back = opt.coef_idt*F.l1_loss(B*(1-M_B), GB_b)
            loss_G_idt_pert_back = opt.coef_idt*F.l1_loss(TB.detach()*(1-M_TB.detach()), GTB_b)

            loss_G_recons = opt.coef_recons*F.l1_loss(A*(1-M_A), GA*(1-M_A))
            
            loss_G = loss_G_GA + loss_G_GTA + gta_tga_distance_G + gta_tga_distance_G_mask + loss_G_TM_GA + loss_G_TM_GTA + loss_G_idt + loss_G_idt_pert + loss_G_idt_pert + loss_G_idt_mask + loss_G_idt_mask_pert + loss_G_GA_back + loss_G_GTA_back + gta_tga_distance_G_back + loss_G_idt_back + loss_G_idt_pert_back + loss_G_recons
            loss_G.backward()
            ...
            # Training D
            set_requires_grad([netD, netP, netD_mask, netD_back], True)
            netP.zero_grad()
            netD.zero_grad()
            netD_back.zero_grad()
            netD_mask.zero_grad()
            ...
            logits_B_back = netD_back(B*(1-M_B))
            logits_GA_back = netD_back(GA_b.detach())
            logits_TB_back = netD_back(TB*(1-M_TB))
            logits_GTA_back = netD_back(GTA_b)
            gta_tga_distance_D_back = opt.coef_mspc*F.l1_loss(GTA_b.detach(), TGA*(1-TM_TGA))

            loss_D_B_back = opt.coef_adv*loss_fn(logits_B_back, target_is_real=True)
            loss_D_GA_back = opt.coef_adv*loss_fn(logits_GA_back, target_is_real=False)
            loss_D_TB_back = opt.coef_adv*loss_fn(logits_TB_back, target_is_real=True)
            loss_D_GTA_back = opt.coef_adv*loss_fn(logits_GTA_back, target_is_real=False)
            
            loss_D = loss_D_B + loss_D_GA + loss_D_TB + loss_D_GTA + gta_tga_distance_D + gta_tga_distance_D_mask + loss_pert_constraint_D + loss_D_M_B + loss_D_TM_GA + loss_D_M_TB + loss_D_TM_GTA + loss_D_B_back + loss_D_GA_back + loss_D_TB_back + loss_D_GTA_back + gta_tga_distance_D_back

            loss_D.backward()
            if opt.use_grad_clip: torch.nn.utils.clip_grad_norm_(netD.parameters(), opt.grad_clip_val)
            if opt.use_grad_clip: torch.nn.utils.clip_grad_norm_(netD_back.parameters(), opt.grad_clip_val)
            ...
            if total_step%opt.eval_freq==0 or total_step==opt.steps:
                ...
                if fid_score <= best_fid:
                    ...
                    torch.save({
                        ...
                        'netD_back_state_dict': netD_back.state_dict(),
                        ...
                        'optimD_back_state_dict': optimD_back.state_dict(),
                        ...
                        'schedulerD_back_state_dict': schedulerD_back.state_dict(),
