...
def train(opt_path):
    ...
    log_items_val = [
        'total_step', 'fid_score_A', 'fid_score_B'
    ]
        ...
        for i, data in enumerate(train_loader):
            ...
            if total_step%opt.eval_freq==0:
                ...
                for i, data in enumerate(val_loader):
                    ...
                    A, B, GA, GB = map(lambda x: tensor2ndarray(x)[0,:h,:w,:], [A, B, GA, GB])
                    ...
                    out_dir_GB = os.path.join(image_out_dir, f'{str(total_step).zfill(len(str(opt.steps)))}', 'GB')

                    os.makedirs(out_dir_A, exist_ok=True)
                    os.makedirs(out_dir_B, exist_ok=True)
                    os.makedirs(out_dir_GA, exist_ok=True)
                    os.makedirs(out_dir_GB, exist_ok=True)
                    A, B, GA, GB = map(lambda x: Image.fromarray(x), [A, B, GA, GB])
                    ...
                    GB.save(os.path.join(out_dir_GB, f'{i:04}.{opt.save_extention}'))
                    if opt.save_compare:
                        ...
                        arrange_images([A, GA, B, GB]).save(os.path.join(out_dir_compare, f'{i:04}.{opt.save_extention}'))
                netG.train()
                
                fid_score_A = get_fid([out_dir_GB, out_dir_A], batch_size=32, dims=2048, num_workers=2) 
                fid_score_B = get_fid([out_dir_GA, out_dir_B], batch_size=32, dims=2048, num_workers=2) 
                    
                txt = f'FID: {fid_score_A:f}, {fid_score_B:f}'
                # print(txt)
                with open(log_path, mode='a', encoding='utf-8') as fp:
                    fp.write(txt+'\n')
                lg = ''
                ...
                if fid_score_B <= best_fid:
                    best_fid = fid_score_B
                    torch.save({
