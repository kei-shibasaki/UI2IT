...
def train(opt_path):
    ...
    netAB = getattr(network_module_G, opt.model_type_G)(opt.netAB).to(device)
    ...
    optimG = torch.optim.Adam(netAB.parameters(), lr=opt.learning_rate_G, betas=opt.betas)
    ...
    if opt.pretrained_path:
        ...
        netAB.load_state_dict(state_dict['netAB_state_dict'], strict=True)
    ...
    for e in range(1, 42*opt.steps):
        for i, data in enumerate(train_loader):
            ...
            loss_G.backward()
            if opt.use_grad_clip: torch.nn.utils.clip_grad_norm_(netAB.parameters(), opt.grad_clip_val)
            optimG.step()
            schedulerG.step()
            ...
            if total_step%opt.eval_freq==0:
                ...
                if fid_score <= best_fid:
                    ...
                    torch.save({
                        ...
                        'netAB_state_dict': netAB.state_dict(),
