...
from models.mspc import PerturbationNetworkMask, grid_sample
...
def train(opt_path):
    ...
    netP = PerturbationNetworkMask(**opt.netP).to(device)
    ...
    for e in range(1, 42*opt.steps):
        for i, data in enumerate(train_loader):
            A = data['mask_A'].to(device)
            B = data['mask_B'].to(device)
            img_A = data['A'].to(device)
            # G(A)
            GA = netG(torch.cat([A,img_A], dim=1))
            # T(A)
            grid_A, TA, constraint_A, cordinate_contraint_A = netP(A)
            img_TA = grid_sample(img_A, grid_A.detach())
            # G(T(A))
            GTA = netG(torch.cat([TA.detach(), img_TA.detach()], dim=1))
            # T(G(A))
            ...
            GTA = netG(torch.cat([TA, img_TA], dim=1))
            ...
            if total_step%opt.eval_freq==0:
                ...
                for i, data in enumerate(val_loader):
                    img_A = data['img'].to(device)
                    A = data['mask'].to(device)
                    b,c,h,w = A.shape
                    A = pad_tensor(A, divisible_by=2**3)
                    img_A = pad_tensor(img_A, divisible_by=2**3)
                    with torch.no_grad():
                        GA = netG(torch.cat([A, img_A], dim=1))
                        grid_A, TA, constraint_A, cordinate_contraint_A = netP(A)
                        img_TA = grid_sample(img_A, grid_A)
                        GTA = netG(torch.cat([TA, img_TA], dim=1))
                        TGA = grid_sample(GA, grid_A)

                    images_save = [A, GA, TA, GTA, TGA]
                    labels = ['A', 'GA', 'TA', 'GTA', 'TGA']
