...
def train(opt_path):
    ...
    log_items_train = [
        'total_step','lr', 'loss_G_GA', 'loss_G_GTA', 'gta_tga_distance_G', 'loss_G_background_recons', 'loss_G_mask_recons', 'loss_G',
    ...
    optimT = torch.optim.Adam(netT.parameters(), lr=opt.learning_rate, betas=opt.betas)
    schedulerT = LinearLRWarmup(optimT, opt.lr_w, opt.lr_max, opt.lr_min, opt.step_w, opt.step_max)
    ...
    if opt.pretrained_path:
        ...
        netT.load_state_dict(state_dict['netT_state_dict'], strict=True)
        ...
        optimT.load_state_dict(state_dict['optimT_state_dict'])
        ...
        schedulerT.load_state_dict(state_dict['schedularT_state_dict'])

    train_dataset = UnpairedImageDatasetWithMask(opt.trainA_path, opt.trainA_mask_path, opt.trainB_path, opt.input_resolution, opt.data_extention, opt.mask_extention, opt.cache_images)
    ...
    for e in range(1, 42*opt.steps):
        for i, data in enumerate(train_loader):
            ...
            # G(T(A))
            TM_GTA = netT(TA.detach(), M_TA.detach())
            GTA_f, GTA_b, GTA = netG(TA.detach(), M_TA.detach(), TM_GTA)
            # T(G(A))
            ...
            loss_G_background_recons = opt.coef_recons*F.l1_loss(GA_b*(1-TM_GA), A*(1-TM_GA))
            loss_G_mask_recons = opt.coef_mask_recons*F.l1_loss(M_A, TM_GA)
            
            loss_G = loss_G_GA + loss_G_GTA + gta_tga_distance_G + loss_G_background_recons + loss_G_mask_recons
            loss_G.backward()
